...
float theFloatingpointNumbers()//生成随机小数
{
    int  number;
    float numberone;
    number = rand() % 100;
...
void arithmetic(int n, int k, int l)//产生随机数和随机符号
{
...
int Generationofarray(int a, int b, int c, int d)//生成符号数组
{
    int sum = 0;
    if (Addition == 1)
    {
...
int main()
{
    ...
    cout << "请输入需要生成题目的数量" << ' ';
    cin >> number;
    cout << "所生成的运算是否包含加法，如果是请输入1，否则输入0" << ' ';
    cin >> a;
    if (a == 1)
    {
        Addition = 1;
    }
    cout << "所生成的运算是否包含减法，如果是请输入1，否则输入0" << ' ';
    cin >> b;
    if (b == 1)
    {
        subtraction = 1;
    }
    cout << "所生成的运算是否包含乘法，如果是请输入1，否则输入0" << ' ';
    cin >> c;
    if (c == 1)
    {
        multiplication = 1;
    }
    cout << "所生成的运算是否包含除法，如果是请输入1，否则输入0" << ' ';
    cin >> d;
    if (d == 1)
    {
        division = 1;
    }
    cout << "请确认随机产生的最大数值" << endl;
    cout << "十以内请输入1 " << endl;
    cout << "100以内请输入2" << endl;
    cin >> l;
    cout << "所生成的运算是否包含括号，如果是请输入1，否则输入0" << ' ';
    cin >> j;
    cout << "所生成的运算是否包含浮点数，如果是请输入1，否则输入0" << ' ';
    cin >> x;
    if (j == 1)
    {
        parentheses = 1;
